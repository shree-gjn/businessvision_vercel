...
const JobSearch = () => {
  ...
  const handleSelectAll = (prefecture) => (event) => {
    const allChecked = Object.keys(checkboxes[prefecture]).reduce((acc, area) => {
      acc[area] = event.target.checked;
      return acc;
    }, {});
    setCheckboxes({
      ...checkboxes,
      [prefecture]: allChecked,
    });
  };
      ...
      <Box>
        <Grid container spacing={2} sx={{padding: '0'}}>
          ...
          <Grid item xs={12} sx={{marginTop: '20px'}}>
            ...
            <Item sx={{padding: '0'}}>
              <FormControl sx={{display: 'flex', justifyContent: 'space-between', gap: '20px', flexDirection: 'row', textAlign: 'left'}} size="small">
                <Select
                  id="upperlimit"
                  value={upperlimit}
          ...
          <Grid item xs={12} sx={{marginTop: '20px'}}>
            ...
            <Item>
              <RadioGroup
                ...
                >
                <FormControlLabel sx={{fontSize: '12px'}} value="希望する業種" control={<Radio />} label="希望する業種" />
                <FormControlLabel sx={{fontSize: '12px'}} value="希望しない業種" control={<Radio />} label="希望しない業種" />
                <FormControlLabel sx={{fontSize: '12px'}} value="こだわらない" control={<Radio />} label="こだわらない" />
              </RadioGroup>
            </Item>
          </Grid>
