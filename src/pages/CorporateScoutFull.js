...
export default function CorporateFull() {
  ...
  return (
    <>
      ...
      {/* <BackLink to="#" onClick={goBack} > <BackButton /> 戻る </BackLink> */}
      <Box sx={{ flexGrow: 1 , padding:'10px', overflow:'scroll', marginTop: '10px'}}>
        <Typography variant="h6" component="div" sx={{textAlign:'left', fontSize:'12px', paddingBottom: '5px'}}> 企業からのコメント </Typography>
...
今後の事業拡大に伴い、経理部門強化の採用です。グループ全社の経理機能を担っている経理部において、経理部マネージャー（管理職）として、月次・四半期・年次含むIFRS（国際会計基準）会計関連の業務をご担当いただきます。​</Item>
        ...
        <Grid container spacing={1} style={{position: 'fixed', bottom: '0', marginBottom: '56px', padding: '15px', background: 'rgb(255 255 255 / 87%)'}}>
          ...
          <Grid item xs={6}>
            <Button component={Link} to="#" variant="contained" color="primary" sx={{fontSize: '12px', width: '100%'}}> 書類選考から エントリー </Button>
          </Grid>
          <Grid item xs={6}>
            <Button component={Link} to="#" variant="contained" color="primary" sx={{fontSize: '12px', width: '100%'}}> 気になる済 </Button>
          </Grid>
          <Grid item xs={6}>
            <Button component={Link} to="#" variant="contained" color="primary" sx={{fontSize: '12px', width: '100%'}}> 辞退する </Button>
          </Grid>
        </Grid>
        </Box>
      </Card>
